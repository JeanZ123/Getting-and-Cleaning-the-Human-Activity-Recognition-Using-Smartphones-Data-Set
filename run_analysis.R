run_analysis <- function() {
  ...
  features <-
    read.table("UCI HAR Dataset\\features.txt", col.names = c("N", "Feature"))
  activities <-
  ...
  subject_test <-
    read.table("UCI HAR Dataset\\test/subject_test.txt",
               col.names = c("Subject"))
  x_test <-
    read.table("UCI HAR Dataset\\test\\X_test.txt", col.names = features$Feature)
  y_test <-
    read.table("UCI HAR Dataset\\test\\y_test.txt", col.names = "Code")
  subject_train <-
    read.table("UCI HAR Dataset\\train/subject_train.txt",
               col.names = c("Subject"))
  x_train <-
    read.table("UCI HAR Dataset\\train\\X_train.txt", col.names = features$Feature)
  y_train <-
    read.table("UCI HAR Dataset\\train\\y_train.txt", col.names = "Code")
  ...
  Filtered_Data <-
    select(Dataset, Subject, Code, contains("mean"), contains("std"))
